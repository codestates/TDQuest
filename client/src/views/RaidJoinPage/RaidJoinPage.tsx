...
import {
  getMonsterInfo,
  MonstersInfo,
} from "../../features/raidjoin/raidjoinSlice";
...
function RaidJoinPage() {
  ...
  const [leftTime, setLeftTime] = useState(
    getLeftTime(
      initialTime.getDay() === 6 || initialTime.getDay() === 0,
      initialTime
    )
  );
  const [canRaid, setCanRaid] = useState(
    initialTime.getDay() === 6 || initialTime.getDay() === 0
  );
  ...
  const handleParticipate = (monsterId: number) => {
    let local = window.localStorage.getItem("isLogin")
      ? JSON.parse(window.localStorage.getItem("isLogin") || "")
      : false;
    setShowToast(true);
  ...
  const participate = async (monsterId: number) => {
    try {
      const paricipateFunc = await TDQuestAPI.post(
        `/raids/invite?user_id=${isLogin.userInfo.id}&raid_id=${monsterId}`
      );
      console.log(paricipateFunc);
      const damage_logInfo = await TDQuestAPI.get(
        `/raids/damage_logs?raid_id=${monsterId}`
      );
  ...
  useEffect(() => {
    ...
    let newisLogin = window.localStorage.getItem("isLogin")
      ? JSON.parse(window.localStorage.getItem("isLogin") || "")
      : false;
            ...
            <SlideContainer ref={slidePage}>
              {monsterInfo.monsterInfo
                ? monsterInfo.monsterInfo.map((el: any) => (
                    <RaidInfoBox
                      name={el.monsterInfo.name}
                      hp={el.monsterInfo.hp}
                      reward={el.monsterInfo.reward}
                      image={el.monsterInfo.monster_image}
                      kind={el.monsterInfo.kind}
                    ></RaidInfoBox>
                  ))
                : null}
          ...
          <Foot>
            <span>
              <Button
                ...
                height={"60px"}
                fontSize={"32px"}
                onClick={() => handleParticipate(currentPage.id)}
              />
              <img src={require("../../static/images/HelperBear.png")} />
            </span>
            {canRaid ? `Raid Start Time Left` : `Until Raid Time`} :{" "}
            {`${leftTime.days}d ${leftTime.hours}h ${leftTime.mins}min`}
          </Foot>
